...
// 处理 HTTP 请求的导出函数
func GrabLatestRSS(w http.ResponseWriter, r *http.Request) {
    ...
    if err != nil {
        logError(config, fmt.Sprintf("[%s] [Read RSS feeds error] %v", getBeijingTime().Format("Mon Jan 2 15:04:2006"), err))
        http.Error(w, "Error reading RSS feeds from GitHub", http.StatusInternalServerError)
        return
    }
    ...
    if err != nil {
        logError(config, fmt.Sprintf("[%s] [Fetch RSS error] %v", getBeijingTime().Format("Mon Jan 2 15:04:2006"), err))
        http.Error(w, "Error fetching RSS feeds", http.StatusInternalServerError)
        return
    }
    ...
    if err != nil {
        logError(config, fmt.Sprintf("[%s] [Save data to GitHub error] %v", getBeijingTime().Format("Mon Jan 2 15:04:2006"), err))
        http.Error(w, "Error saving data to GitHub", http.StatusInternalServerError)
        return
    }

    w.WriteHeader(http.StatusOK)
    w.Write([]byte("RSS feeds successfully fetched and saved to GitHub!"))
}

func main() {
    http.HandleFunc("/api/grabLatestRSS", GrabLatestRSS)
