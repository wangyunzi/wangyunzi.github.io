...
```go
{% raw %}.Site.OutputFormats.Get("RSS").Permalink{% endraw %}
```
...
```go
{% raw %}<a href="{{ .Site.OutputFormats.Get('RSS').Permalink }}" type="application/rss+xml" title="rss" target="_blank"><i class="ri-rss-fill"></i></a>{% endraw %}
```
